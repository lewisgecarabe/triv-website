        ...
        switch ($_POST['action']) {
            case 'create':
                ...
                if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                    ...
                    $fileName = 'team_' . uniqid() . '.' . $fileExtension;
            ...
            case 'update':
                ...
                if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadPath)) {
                        ...
                        $oldTeamMember = $teamMemberObj->getById($_POST['team_member_id']);
                        if ($oldTeamMember && !empty($oldTeamMember['image'])) {
                            $oldImagePath = $uploadDir . $oldTeamMember['image'];
                ...
                if ($teamMemberObj->update($_POST['team_member_id'], $data)) {
            ...
            case 'delete':
                $imageFile = $teamMemberObj->delete($_POST['team_member_id']);
...
$teamMembers = $teamMemberObj->getAll();

// Get team member for editing if ID is provided
$editTeamMember = null;
if (isset($_GET['edit'])) {
    $editTeamMember = $teamMemberObj->getById($_GET['edit']);
}
?>
...
<head>
    ...
    <title>Manage Team Members - TRIV Admin</title>
...
<body>
    <aside>
        ...
        <ul>
            ...
            <li><a href="manage-team.php" class="active"><i class="fas fa-users-cog"></i> Team Members</a></li>
    ...
    <main>
        <div class="header">
            <h1><i class="fas fa-users-cog"></i> Manage Team Members</h1>
            <p>Add, edit, and manage team members displayed on the About Us page</p>
        </div>
        ...
        <div class="form-container">
            <h2><?= $editTeamMember ? 'Edit Team Member' : 'Add New Team Member' ?></h2>
            <form method="POST" enctype="multipart/form-data">
                <input type="hidden" name="action" value="<?= $editTeamMember ? 'update' : 'create' ?>">
                <?php if ($editTeamMember): ?>
                    <input type="hidden" name="team_member_id" value="<?= $editTeamMember['id'] ?>">
                <?php endif; ?>

                <div class="form-row">
                    <div class="form-group">
                        <label for="name">Name *</label>
                        <input type="text" id="name" name="name" value="<?= $editTeamMember ? htmlspecialchars($editTeamMember['name']) : '' ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="position">Position *</label>
                        <input type="text" id="position" name="position" value="<?= $editTeamMember ? htmlspecialchars($editTeamMember['position']) : '' ?>" required>
                    </div>
                </div>

                <div class="form-group">
                    <label for="bio">Bio</label>
                    <textarea id="bio" name="bio"><?= $editTeamMember ? htmlspecialchars($editTeamMember['bio']) : '' ?></textarea>
                </div>

                <div class="form-row">
                    <div class="form-group">
                        <label for="specialization">Specialization</label>
                        <input type="text" id="specialization" name="specialization" value="<?= $editTeamMember ? htmlspecialchars($editTeamMember['specialization']) : '' ?>">
                    </div>
                    <div class="form-group">
                        <label for="years_experience">Years of Experience</label>
                        <input type="number" id="years_experience" name="years_experience" value="<?= $editTeamMember ? htmlspecialchars($editTeamMember['years_experience']) : '0' ?>" min="0">
                    </div>
                </div>

                <div class="form-row">
                    <div class="form-group">
                        <label for="image">Profile Image <?= $editTeamMember ? '(Leave empty to keep current image)' : '' ?></label>
                        <input type="file" id="image" name="image" accept="image/*" <?= $editTeamMember ? '' : 'required' ?>>
                        <?php if ($editTeamMember && !empty($editTeamMember['image'])): ?>
                            <p style="margin-top: 5px;">Current image:</p>
                            <img src="../assets/images/<?= htmlspecialchars($editTeamMember['image']) ?>" alt="Current profile" class="preview-image">
                        <?php endif; ?>
                    </div>
                    <div class="form-group">
                        <label for="order_position">Display Order</label>
                        <input type="number" id="order_position" name="order_position" value="<?= $editTeamMember ? htmlspecialchars($editTeamMember['order_position']) : '0' ?>" min="0">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="email" id="email" name="email" value="<?= $editTeamMember ? htmlspecialchars($editTeamMember['email']) : '' ?>">
                    </div>
                    <div class="form-group">
                        <label for="phone">Phone</label>
                        <input type="text" id="phone" name="phone" value="<?= $editTeamMember ? htmlspecialchars($editTeamMember['phone']) : '' ?>">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="linkedin" name="linkedin" value="<?= $editTeamMember ? htmlspecialchars($editTeamMember['linkedin']) : '' ?>">
                    </div>
                    <div class="form-group">
                        <label for="status">Status *</label>
                        <select id="status" name="status" required>
                            <option value="active" <?= $editTeamMember && $editTeamMember['status'] === 'active' ? 'selected' : '' ?>>Active</option>
                            <option value="inactive" <?= $editTeamMember && $editTeamMember['status'] === 'inactive' ? 'selected' : '' ?>>Inactive</option>
                        </select>
                    </div>
                </div>
                ...
                <div style="display: flex; gap: 10px;">
                    <button type="submit" class="btn btn-success">
                        <i class="fas fa-save"></i> <?= $editTeamMember ? 'Update Team Member' : 'Add Team Member' ?>
                    </button>
                    <?php if ($editTeamMember): ?>
                        <a href="manage-team.php" class="btn btn-secondary">
                        ...
                        <tr>
                            <td>
                                <?php if (!empty($member['image'])): ?>
                                    <img src="../assets/images/<?= htmlspecialchars($member['image']) ?>" alt="<?= htmlspecialchars($member['name']) ?>" style="width: 50px; height: 50px; object-fit: cover; border-radius: 50%;">
                                <?php else: ?>
                            ...
                            <td><?= htmlspecialchars($member['specialization']) ?></td>
                            <td><?= $member['years_experience'] ?> years</td>
